...
13. "Rate Legislator" and "Share Content" are still listed in `ACTION_POINTS`/`ALL_ACTIONS` (with achievements attached) but have no UI anywhere that triggers them — confirmed via full-codebase search, zero references outside the gamification constants themselves. Decide before touching: either build the two features (rating writes to the already-declared-but-unused `legislatorRatings` Firestore collection; share needs the native Share API) or trim these entries out of the catalog
14. A throwaway test account exists in the production Firebase project from manual verification: `claude-test-verification-20260704@example.com` ("ClaudeTest Verification"), used to confirm the Profile → Points navigation live. Safe to delete from Firebase Auth + its `users/{uid}` Firestore doc whenever
