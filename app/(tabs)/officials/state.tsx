...
type SortOption = 'nameAsc' | 'nameDesc' | 'districtAsc' | 'districtDesc' | 'partyAsc' | 'chamberAsc';
...
const SORT_OPTIONS: { value: SortOption; label: string }[] = [
  ...
  { value: 'chamberAsc', label: 'Chamber (A-Z)' },
];

const sortLegislators = (items: Official[], sortBy: SortOption): Official[] => {
  const sorted = [...items];
  switch (sortBy) {
    case 'nameAsc':
    ...
    case 'chamberAsc':
      return sorted.sort(
        (a, b) => a.chamber.localeCompare(b.chamber) || a.familyName.localeCompare(b.familyName),
      );
    default:
      return sorted;
  }
...
export default function StateLegislatorsScreen() {
  ...
  const fetchLegislators = async () => {
    try {
      ...
      setError(message);
      AppAlert.alert('Error', `Unable to load Kansas legislators.\n\n${message}`);
      setLegislators([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return sortLegislators(result, sortBy);
