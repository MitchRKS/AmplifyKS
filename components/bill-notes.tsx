...
interface BillNotesProps {
  /** LegiScan numeric bill_id — the same key the iOS BillNotesManager uses. */
  billId: number;
}

// Private per-bill note, one doc per bill at users/{uid}/billNotes/{billId}.
// The doc shape ({ billId: number, text }) matches the iOS app's
// BillNotesManager exactly so notes sync across platforms; saving empty text
// deletes the note, same as iOS.
export function BillNotes({ billId }: BillNotesProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const snap = await getDoc(doc(db, 'users', uid, 'billNotes', String(billId)));
        ...
        console.error('Error loading bill note:', e);
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      const db = getFirestoreDb();
      const noteRef = doc(db, 'users', user.uid, 'billNotes', String(billId));
      if (trimmed) {
      ...
      console.error('Error saving bill note:', e);
    ...
    <View style={[styles.card, { borderColor: border }]}>
      ...
      {!user ? (
        <ThemedText style={[styles.signInHint, { color: mutedText }]}>
          Sign in to keep private notes about this bill
        </ThemedText>
      ) : loading ? (
        ...
        <>
          <TextInput
            ...
            placeholder="Add a note — positions, testimony ideas, follow-ups…"
